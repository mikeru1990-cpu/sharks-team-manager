...
function Dashboard({
  ...
  async function loadAll() {
    ...
    const [playersRes, settingsRes, eventsRes, attendanceRes, coachesRes, coachAvailabilityRes] =
      await Promise.all([
    ...
    if (!playersRes.error && playersRes.data && playersRes.data.length > 0) {
      const nextPlayers: Player[] = playersRes.data.map((row: any) => ({
        id: row.id,
        name: row.name,
        positions: parseJsonArray(row.positions_json) as Player["positions"],
        mainGK: !!row.main_gk,
  ...
  async function loadMatchState(eventId: string | null) {
    ...
    if (!stateRes.error && stateRes.data) {
      ...
      setLiveSecondsMap(parseJsonObject(stateRes.data.live_seconds_json))
      setLineupMap(parseJsonObject(stateRes.data.lineup_json))
      setBenchIds(parseJsonArray(stateRes.data.bench_json) as string[])
    } else {
      const fallbackSlots = buildPitchSlots("7v7", "2-3-1")
      const auto = buildAutoLineup(matchPlayers.length > 0 ? matchPlayers : players, fallbackSlots)
    ...
    if (!lineupsRes.error && lineupsRes.data) {
      setSavedLineups(
        lineupsRes.data.map((row: any) => ({
          ...
          lineup: parseJsonObject(row.lineup_json),
          bench: parseJsonArray(row.bench_json) as string[],
        }))
      )
    } else {
      setSavedLineups([])
  ...
  function openEditCalendarEvent(event: EventItem) {
    ...
    setEventType(
      event.type === "training" || event.type === "match" || event.type === "other"
        ? event.type
        : "training"
    )
  ...
  async function handleEndGameAndSaveResult() {
    if (!supabase || !activeMatchEventId || !activeMatchEvent || !isAdmin) return

    const confirmed = window.confirm("End game and save final result?")
    if (!confirmed) return
    ...
    const { error } = await supabase
      .from("events")
      .update({
        notes: `${activeMatchEvent.notes ? `${activeMatchEvent.notes}\n` : ""}FINAL RESULT: ${homeTeam} ${homeScore} - ${awayScore} ${awayTeam}`,
      })
    ...
    alert("Game ended and result saved")
  }
              ...
              <div style={cardStyle()}>
                ...
                >
                  <div>
                    ...
                    {selectedEvent.notes ? (
                      <div style={{ color: "#475569", marginTop: 8, whiteSpace: "pre-wrap" }}>{selectedEvent.notes}</div>
                    ) : null}
                  </div>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            {selectedDateCoachAvailability.length > 0 ? (
              <div style={cardStyle("#eff6ff")}>
                <div style={{ fontSize: 22, fontWeight: 900, marginBottom: 10 }}>Coach Availability Snapshot</div>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            ...
            {isAdmin && activeMatchEvent ? (
              <div style={cardStyle("#ecfccb")}>
                <div style={{ fontSize: 22, fontWeight: 900, marginBottom: 10 }}>End Game</div>
                <div style={{ color: "#3f6212", marginBottom: 12 }}>
                  Save final result for <strong>{activeMatchEvent.title}</strong>
                </div>
                <button
                  onClick={() => void handleEndGameAndSaveResult()}
                  style={{
                    ...buttonPrimary(),
                    background: "#16a34a",
                  }}
