...
function cleanWarnings(warnings: string[], ignoredPlayer?: Player | null) { return warnings.filter((warning) => !ignoredPlayer || !normalise(warning).includes(normalise(ignoredPlayer.name))) }
function clamp(value: number, min: number, max: number) { return Math.max(min, Math.min(max, value)) }

function RotationHealth({ savedPeriods, fairnessScore, lowRows, benchRows, shortLabel, averageMinutes, warnings, goalkeeperMode }: { savedPeriods: number; fairnessScore: number; lowRows: PlayerMinuteRow[]; benchRows: PlayerMinuteRow[]; shortLabel: string; averageMinutes: number; warnings: string[]; goalkeeperMode: GoalkeeperMode }) {
...
export default function QuarterPlanner({ isAdmin, currentQuarter, setCurrentQuarter, quarterPlans, quarterWarnings, currentSlots, players, lineupMap, benchIds, onSaveCurrentQuarter, onLoadQuarter, onAutoGenerate, periodMode, periodLength }: Props) {
  ...
  const currentGoalkeeper = detectedGoalkeeper?.name || "Empty"
