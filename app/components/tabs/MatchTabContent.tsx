...
function MatchCommandCard({ event, activeMatchEventId, events, smartMatch, setActiveMatchEventId, persistSettings, setMatchTab, available, maybe, unavailable, coaches, running }: {
  event: EventWithPlan | null
  activeMatchEventId: string | null
  events: EventWithPlan[]
  smartMatch: EventWithPlan | null
  setActiveMatchEventId: (value: string | null) => void
  persistSettings: (patch?: Partial<{ activeMatchEventId: string | null }>) => Promise<void>
  ...
  const matchEvents = events.filter((item) => item.type === "match")
    ...
    <div className="sharks-elite-panel sharks-card-shine" style={{ borderRadius: 24, padding: 14, display: "grid", gap: 12, overflow: "hidden" }}>
      ...
      {matchEvents.length ? (
        <select value={selectedValue} onChange={(changeEvent) => chooseMatch(changeEvent.target.value || null)} style={{ padding: 11, borderRadius: 15, border: "1px solid rgba(125,211,252,0.24)", fontSize: 14, width: "100%", background: "rgba(2,6,23,0.66)", color: "white", fontWeight: 900 }}>
          ...
          {matchEvents.filter(isThisWeek).length ? <optgroup label="This week">{matchEvents.filter(isThisWeek).map((item) => <option key={item.id} value={item.id}>{formatShortDate(item.date)} • {item.startTime || "00:00"} • {item.title}</option>)}</optgroup> : null}
          <optgroup label="Other matches">{matchEvents.filter((item) => !isThisWeek(item)).slice().sort((a, b) => `${a.date}${a.startTime || ""}`.localeCompare(`${b.date}${b.startTime || ""}`)).map((item) => <option key={item.id} value={item.id}>{formatShortDate(item.date)} • {item.startTime || "00:00"} • {item.title}</option>)}</optgroup>
        </select>
      ...
      <div style={{ borderRadius: 16, padding: "10px 12px", background: "rgba(2,6,23,0.52)", border: "1px solid rgba(125,211,252,0.18)", color: "#e5eefc", fontWeight: 900, fontSize: 13, display: "flex", gap: 10, flexWrap: "wrap" }}>
        <span style={{ color: "#86efac" }}>Available {available}</span>
        <span style={{ color: "#fde68a" }}>Maybe {maybe}</span>
        <span style={{ color: "#fecdd3" }}>Out {unavailable}</span>
        <span style={{ color: "#7dd3fc" }}>Coaches {coaches}</span>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 8 }}>
        <PrimaryButton onClick={() => setMatchTab("overview")}>Match</PrimaryButton>
        <SecondaryButton onClick={() => setMatchTab("lineup")}>Lineup</SecondaryButton>
        <SecondaryButton onClick={() => setMatchTab("quarters")}>Quarters</SecondaryButton>
      </div>
    </div>
  )
}
...
export default function MatchTabContent(props: Props) {
  ...
  useEffect(() => {
    if (matchTab !== "overview" && !running && !shouldShowPostMatch) return
    if (!matchTab) setMatchTab("overview")
  }, [matchTab, running, shouldShowPostMatch, setMatchTab])
