...
import { getActiveU11Players, leonardStanleyPlayers } from "../../lib/realTeamData"

const activePlayers = getActiveU11Players()
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 12 }}>
        <Summary label="U11 Players" value={activePlayers.length.toString()} />
        <Summary label="Pending" value={pendingPlayers.length.toString()} />
      </div>

      <section style={panel}>
        <h2 style={{ margin: 0, fontSize: 24 }}>U11 Girls Squad</h2>
      ...
      <section style={panel}>
        <h2 style={{ margin: 0, fontSize: 24 }}>Pending / To Confirm</h2>
        <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
          {pendingPlayers.map((player) => (
            <article key={player.id} style={card}>
              <div style={{ fontSize: 21, fontWeight: 950 }}>{player.name}</div>
              <div style={{ marginTop: 4, color: "#fbbf24", fontWeight: 900 }}>Pending</div>
            </article>
          ))}
        </div>
      </section>
...
function Mini({ label, value }: { label: string; value: string }) {
  return <div style={{ borderRadius: 16, padding: 12, background: "rgba(15,23,42,0.84)" }}><div style={{ fontSize: 11, color: "rgba(226,232,240,0.58)", fontWeight: 900 }}>{label}</div><div style={{ marginTop: 6, fontWeight: 950 }}>{value}</div></div>
}
