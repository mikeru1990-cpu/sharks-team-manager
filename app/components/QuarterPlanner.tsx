...
function chip(active: boolean) {
  return { border: active ? "1px solid rgba(125,211,252,0.75)" : "1px solid rgba(148,163,184,0.22)", background: active ? "rgba(14,165,233,0.22)" : "rgba(255,255,255,0.06)", color: active ? "#7dd3fc" : "#cbd5e1", borderRadius: 14, padding: "10px 12px", fontWeight: 1000 as const, fontSize: 13, cursor: "pointer" }
}
...
function findLockedGoalkeeper(players: Player[], currentSlots: PitchSlot[], lineupMap: Record<string, string | null>) {
  ...
  return players.find((player) => player.id === currentGkId) || players.find((player) => player.mainGK) || players.find((player) => normalise(player.name).includes("darcy-rae")) || null
}

function cleanWarnings(warnings: string[], lockedGoalkeeper?: Player | null) {
  return warnings.filter((warning) => !lockedGoalkeeper || !normalise(warning).includes(normalise(lockedGoalkeeper.name)))
}
...
function RotationHealth({ savedPeriods, fairnessScore, lowRows, benchRows, shortLabel, averageMinutes, warnings }: { savedPeriods: number; fairnessScore: number; lowRows: PlayerMinuteRow[]; benchRows: PlayerMinuteRow[]; shortLabel: string; averageMinutes: number; warnings: string[] }) {
  if (savedPeriods === 0) {
    return <div style={{ borderRadius: 16, padding: 12, background: "rgba(14,165,233,0.12)", border: "1px solid rgba(56,189,248,0.32)", color: "#bae6fd", fontWeight: 900 }}>Auto-balance or save the rotation to see minutes and fairness checks.</div>
  }
  ...
  const issueNames = [...lowRows.slice(0, 4).map((row) => `${row.player.name} below target (${row.minutes}m vs ${averageMinutes}m)`), ...benchRows.slice(0, 4).map((row) => `${row.player.name} benched ${row.periodsBenched.map((period) => `${shortLabel}${period}`).join(", ")}`)]
  return (
...
export default function QuarterPlanner({ isAdmin, currentQuarter, setCurrentQuarter, quarterPlans, quarterWarnings, currentSlots, players, lineupMap, benchIds, onSaveCurrentQuarter, onLoadQuarter, onAutoGenerate, periodMode, periodLength }: Props) {
  ...
  const onPitchSize = currentSlots.length
  const defaultSquadSize = Math.max(onPitchSize, Math.min(players.length || onPitchSize, onPitchSize + Math.max(benchIds.length, 0)))
  const [matchdaySize, setMatchdaySize] = useState(defaultSquadSize)
  const safeMatchdaySize = Math.max(onPitchSize, Math.min(players.length || onPitchSize, matchdaySize))
  ...
  const matchdayPlayers = useMemo(() => {
    if (!lockedGoalkeeper) return players.slice(0, safeMatchdaySize)
    const outfield = players.filter((player) => player.id !== lockedGoalkeeper.id)
    return [lockedGoalkeeper, ...outfield.slice(0, Math.max(0, safeMatchdaySize - 1))]
  }, [players, lockedGoalkeeper, safeMatchdaySize])
  ...
  const matchdayBenchCount = Math.max(0, safeMatchdaySize - currentFilledCount)
  const incompleteSlots = currentSlots.filter((slot) => !lineupMap[slot.id])
  const quickSizes = Array.from(new Set([onPitchSize, onPitchSize + 1, onPitchSize + 2, 10, 11, players.length].filter((size) => size >= onPitchSize && size <= players.length))).sort((a, b) => a - b)

  return (
    <div style={{ display: "grid", gap: 14 }}>
      <PageCard>
        <SectionHeader title="Matchday Rotation Centre" subtitle="Works across team sizes, formats and matchday squads." />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 8, marginBottom: 12 }}><Stat label="Squad" value={safeMatchdaySize} /><Stat label="Avg" value={`${averageMinutes}m`} /><Stat label="Low mins" value={severeRows.length} colour={severeRows.length ? "#f59e0b" : "#22c55e"} /><Stat label="Bench runs" value={consecutiveRows.length} colour={consecutiveRows.length ? "#ef4444" : "#22c55e"} /></div>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 92px", gap: 8, marginBottom: 10 }}><input type="range" min={onPitchSize} max={Math.max(onPitchSize, players.length)} value={safeMatchdaySize} onChange={(event) => setMatchdaySize(Number(event.target.value))} style={{ width: "100%" }} /><input type="number" min={onPitchSize} max={Math.max(onPitchSize, players.length)} value={safeMatchdaySize} onChange={(event) => setMatchdaySize(Number(event.target.value) || onPitchSize)} style={{ borderRadius: 14, border: "1px solid rgba(148,163,184,0.22)", background: "rgba(2,6,23,0.58)", color: "white", padding: "8px 10px", fontWeight: 1000 }} /></div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>{quickSizes.map((size) => <button key={size} onClick={() => setMatchdaySize(size)} style={chip(safeMatchdaySize === size)}>{size}</button>)}</div>
