...
const U10_NAMES = ["bailee", "elsy", "evelyn", "selena", "selina"]
...
function isYoungerPlayer(player: Player) { const name = normalise(player.name); return U10_NAMES.some((u10) => name.includes(u10)) }
function cleanWarnings(warnings: string[], lockedGoalkeeper?: Player | null) { return warnings.filter((warning) => { const safe = normalise(warning); if (lockedGoalkeeper && safe.includes(normalise(lockedGoalkeeper.name))) return false; return !U10_NAMES.some((name) => safe.includes(name)) }) }
...
function RotationHealth({ savedPeriods, fairnessScore, lowRows, benchRows, shortLabel, averageMinutes, warnings }: { savedPeriods: number; fairnessScore: number; lowRows: PlayerMinuteRow[]; benchRows: PlayerMinuteRow[]; shortLabel: string; averageMinutes: number; warnings: string[] }) {
  if (savedPeriods === 0) return <div style={{ borderRadius: 16, padding: 12, background: "rgba(14,165,233,0.12)", border: "1px solid rgba(56,189,248,0.32)", color: "#bae6fd", fontWeight: 900 }}>Auto-balance or save the rotation to see outfield minutes and fairness checks.</div>
...
export default function QuarterPlanner({ isAdmin, currentQuarter, setCurrentQuarter, quarterPlans, quarterWarnings, currentSlots, players, lineupMap, benchIds, onSaveCurrentQuarter, onLoadQuarter, onAutoGenerate, periodMode, periodLength }: Props) {
  ...
  const activePlayers = players.filter((player) => !isYoungerPlayer(player))
