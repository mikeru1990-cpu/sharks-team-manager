...
function prettyDate(date?: string) { if (!date) return "No date"; try { return new Date(`${date}T12:00:00`).toLocaleDateString("en-GB", { weekday: "short", day: "2-digit", month: "short" }) } catch { return date } }
function fullToday() { try { return new Date().toLocaleDateString("en-GB", { weekday: "long", day: "2-digit", month: "long" }) } catch { return "Today" } }
function nextEvent(events: EventWithPlan[], type?: string) { const today = todayLocal(); return [...events].filter((event) => (!type || event.type === type) && event.date >= today).sort((a, b) => `${a.date}${a.startTime || ""}`.localeCompare(`${b.date}${b.startTime || ""}`))[0] }
function MiniStat({ label, value, colour }: { label: string; value: string | number; colour: string }) { return <div style={{ borderRadius: 18, padding: 12, background: "rgba(2,6,23,0.52)", border: `1px solid ${colour}44`, minHeight: 72 }}><div style={{ color: "#94a3b8", fontSize: 9, fontWeight: 1000, letterSpacing: ".12em", textTransform: "uppercase" }}>{label}</div><div style={{ color: colour, fontSize: 27, fontWeight: 1000, lineHeight: 1, marginTop: 7 }}>{value}</div></div> }
function ActionButton({ label, icon, onClick }: { label: string; icon: string; onClick: () => void }) { return <button onClick={onClick} style={{ border: "1px solid rgba(125,211,252,0.24)", background: "rgba(15,23,42,0.70)", borderRadius: 18, padding: 13, color: "white", fontWeight: 1000, cursor: "pointer", display: "flex", gap: 10, alignItems: "center", justifyContent: "center" }}><span>{icon}</span><span>{label}</span></button> }
function FocusCard({ title, event, empty, onClick }: { title: string; event?: EventWithPlan; empty: string; onClick: () => void }) { return <button onClick={onClick} style={{ textAlign: "left", border: "1px solid rgba(125,211,252,0.20)", background: "rgba(2,6,23,0.46)", borderRadius: 22, padding: 14, display: "grid", gap: 7, cursor: "pointer", color: "white" }}><div style={{ color: "#7dd3fc", fontSize: 10, fontWeight: 1000, letterSpacing: ".14em", textTransform: "uppercase" }}>{title}</div><div style={{ fontSize: 17, fontWeight: 1000, lineHeight: 1.1 }}>{event?.title || empty}</div><div style={{ color: "#cbd5e1", fontSize: 12, fontWeight: 850 }}>{event ? `${prettyDate(event.date)}${event.startTime ? ` • ${event.startTime}` : ""}` : "Tap to add or view events"}</div>{event?.opponent ? <div style={{ color: "#bae6fd", fontSize: 12, fontWeight: 900 }}>vs {event.opponent}</div> : null}</button> }
...
function Notice({ children, tone = "blue" }: { children: React.ReactNode; tone?: "blue" | "yellow" | "green" }) { const colour = tone === "green" ? "#22c55e" : tone === "yellow" ? "#f59e0b" : "#38bdf8"; return <div style={{ borderRadius: 16, padding: 11, background: `${colour}16`, border: `1px solid ${colour}44`, color: "#e5eefc", fontWeight: 850 }}>{children}</div> }
function AssistantCard({ tasks }: { tasks: string[] }) { return <div className="sharks-glass" style={{ borderRadius: 24, padding: 14, display: "grid", gap: 10, border: "1px solid rgba(125,211,252,0.18)" }}><div style={{ color: "#7dd3fc", fontSize: 11, fontWeight: 1000, letterSpacing: ".16em", textTransform: "uppercase" }}>Coach Assistant</div>{tasks.length ? <div style={{ display: "grid", gap: 8 }}>{tasks.slice(0, 5).map((task, index) => <Notice key={task} tone={index === 0 ? "yellow" : "blue"}>• {task}</Notice>)}</div> : <Notice tone="green">All clear. No urgent coaching tasks detected.</Notice>}</div> }
...
export default function FootballHomeDashboard({ teamName, players, events, attendance, results, ratings, activeMatchEventId, onOpenTab }: Props) {
  ...
  const pendingTasks = [!focusEvent ? "Add your next training or match" : null, focusEvent && available === 0 ? "Collect availability responses" : null, match ? "Review matchday squad" : "Plan next session", players.length < 7 ? "Add more squad players" : null, ratings.length === 0 ? "Add your first player ratings" : null].filter(Boolean) as string[]
  ...
  return <section style={{ display: "grid", gap: 14 }}>
    <div className="sharks-elite-panel sharks-card-shine" style={{ borderRadius: 28, padding: 18, display: "grid", gap: 16, overflow: "hidden" }}>
      <div style={{ display: "flex", gap: 14, alignItems: "center", minWidth: 0 }}><ClubBadge size={64} /><div style={{ minWidth: 0 }}><div style={{ color: "#7dd3fc", fontSize: 10, fontWeight: 1000, letterSpacing: ".14em", textTransform: "uppercase" }}>Coach Operating System • {fullToday()}</div><div style={{ color: "white", fontSize: 30, fontWeight: 1000, letterSpacing: "-0.055em", lineHeight: 1, overflowWrap: "anywhere" }}>{teamName}</div><div style={{ color: "#cbd5e1", fontSize: 13, fontWeight: 800, marginTop: 4 }}>Home, matchday, training, squad, insights and club admin in one place.</div></div></div>
    ...
    <AssistantCard tasks={pendingTasks} />
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}><WorkspaceCard icon="⚽" title="Matchday" subtitle="Squad, rotation, live events and summary." onClick={() => onOpenTab("match")} /><WorkspaceCard icon="🏃" title="Training" subtitle="Sessions, attendance and planning." onClick={() => onOpenTab("events")} /><WorkspaceCard icon="👥" title="Squad" subtitle="Profiles, roles and development." onClick={() => onOpenTab("players")} /><WorkspaceCard icon="📊" title="Insights" subtitle={`Wins ${wins} • Avg rating ${avgRating}`} onClick={() => onOpenTab("stats")} /><WorkspaceCard icon="⚙️" title="Club" subtitle="Teams, coaches and settings." onClick={() => onOpenTab("coaches")} /></div>
  </section>
}
